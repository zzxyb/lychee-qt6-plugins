...
class LycheeDecorationPrivate : public QObjectPrivate
{
public:
    LycheeDecorationPrivate();
    ~LycheeDecorationPrivate() override;
...
LycheeDecorationPrivate::LycheeDecorationPrivate()
{
}
...
LycheeDecoration::LycheeDecoration()
    : d(new LycheeDecorationPrivate)
{
    QPalette palette;
    d.data()->m_foregroundColor = palette.color(QPalette::Active, QPalette::WindowText);
    d.data()->m_backgroundColor = palette.color(QPalette::Active, QPalette::Window);
    d.data()->m_foregroundInactiveColor = palette.color(QPalette::Disabled, QPalette::WindowText);

    QTextOption option(Qt::AlignHCenter | Qt::AlignVCenter);
    option.setWrapMode(QTextOption::NoWrap);
    d.data()->m_windowTitle.setTextOption(option);
}
...
QRectF LycheeDecoration::closeButtonRect() const
{
...
QRectF LycheeDecoration::maximizeButtonRect() const
{
    const int windowRight = waylandWindow()->windowContentGeometry().right() + 1;
...
QRectF LycheeDecoration::minimizeButtonRect() const
{
    const int windowRight = waylandWindow()->windowContentGeometry().right() + 1;
...
QMargins LycheeDecoration::margins(MarginsType marginsType) const
{
...
void LycheeDecoration::paint(QPaintDevice *device)
{
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        p.fillPath(roundedRect, d.data()->m_backgroundColor);
        p.restore();
    }
    ...
    if (!windowTitleText.isEmpty()) {
        if (d.data()->m_windowTitle.text() != windowTitleText) {
            d.data()->m_windowTitle.setText(windowTitleText);
            d.data()->m_windowTitle.prepare();
        }
        ...
        p.setPen(active ? d.data()->m_foregroundColor : d.data()->m_foregroundInactiveColor);
        QSizeF size = d.data()->m_windowTitle.size();
        ...
        QPoint windowTitlePoint(top.topLeft().x() + dx,
                 top.topLeft().y() + dy);
        p.drawStaticText(windowTitlePoint, d.data()->m_windowTitle);
        p.restore();
    }
    ...
    // Default pen
    QPen pen(active ? d.data()->m_foregroundColor : d.data()->m_foregroundInactiveColor);
    p.setPen(pen);
    ...
    if ((window()->windowStates() & Qt::WindowMaximized)) {
        ...
        p.drawRect(rect1);
        p.setBrush(d.data()->m_backgroundColor); // need to cover up some lines from the other rect
        p.drawRect(rect2);
    } else {
        p.drawRect(rect);
...
bool LycheeDecoration::clickButton(Qt::MouseButtons b, Button btn)
{
    if (isLeftClicked(b)) {
        d.data()->m_clicking = btn;
        return false;
    } else if (isLeftReleased(b)) {
        if (d.data()->m_clicking == btn) {
            d.data()->m_clicking = None;
            return true;
        } else {
            d.data()->m_clicking = None;
        }
...
bool LycheeDecoration::handleMouse(QWaylandInputDevice *inputDevice, const QPointF &local, const QPointF &global, Qt::MouseButtons b, Qt::KeyboardModifiers mods)
...
bool LycheeDecoration::handleTouch(QWaylandInputDevice *inputDevice, const QPointF &local, const QPointF &global, QEventPoint::State state, Qt::KeyboardModifiers mods)
{
    Q_UNUSED(global);
    QRect wg = waylandWindow()->windowContentGeometry();
...
void LycheeDecoration::processPointerTop(QWaylandInputDevice *inputDevice,
                                                 const QPointF &local,
                                                 Qt::MouseButtons b,
...
void LycheeDecoration::processPointerBottom(QWaylandInputDevice *inputDevice,
                                                      const QPointF &local,
                                                      Qt::MouseButtons b,
                                                      Qt::KeyboardModifiers mods,
                                                      PointerType type)
...
void LycheeDecoration::processPointerLeft(QWaylandInputDevice *inputDevice,
                                                    const QPointF &local,
                                                    Qt::MouseButtons b,
                                                    Qt::KeyboardModifiers mods,
                                                    PointerType type)
...
void LycheeDecoration::processPointerRight(QWaylandInputDevice *inputDevice,
                                                     const QPointF &local,
                                                     Qt::MouseButtons b,
                                                     Qt::KeyboardModifiers mods,
                                                     PointerType type)
